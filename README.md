...
`print_delphi_palettes()` draws a grid of palette swatches, one panel
...
`"grand-budapest"`. `delphi_palette()` returns one as a character vector
of hex colors:
...
Discrete scales (`scale_color_delphi_d()` / `scale_fill_delphi_d()`)
extend a palette by interpolation if there are more levels than colors;
continuous scales (`scale_color_delphi_c()` / `scale_fill_delphi_c()`)
...
`seasonal`, and `artistic`. Pass one to `delphi_palettes()` to see just
that group:
...
protanopia, and tritanopia. `delphi_palette_colorblind()` reports the
verdict for one palette, and
...
this package’s own in [LICENSE.md](LICENSE.md).
